...
void main() {
  ...
  final List<int> releaseDate =
      allMachinerys.map((e) => e.releaseDate.year).toList();
  final currentYear = DateTime.now().year;
  final totalAge = releaseDate
  ...
  final List<int> halfYear =
      sortedDate.take((sortedDate.length / 2).round()).toList();
  final totalOlderAge = halfYear
      .map((year) => currentYear - year)
      .reduce((value, element) => value + element);
